"""
This script clusters audio feature data for 'disco' and 'classical' tracks using K-Means.
It then evaluates the clustering accuracy and visualizes results via PCA.

Steps:
1. Load and filter the dataset to include only 'disco' and 'classical' genres.
2. Convert feature columns to numeric types.
3. Scale features using StandardScaler.
4. Apply K-Means clustering with 2 clusters.
5. Assign cluster labels to the predominant genre in each cluster.
6. Compute accuracy against the original genre labels.
7. Use PCA for 2D visualization of clusters and original labels.
8. Save results, plots, and models for future reference.

Generates:
- 'cluster_visualization_2_clusters.png': PCA plot colored by cluster assignments.
- 'original_label_visualization_2_clusters.png': PCA plot colored by original genres.
- 'audio_features_with_clusters_and_pca_2_clusters.csv': CSV with cluster assignments and PCA components.
- 'scaler_2_clusters.pkl': Fitted StandardScaler.
- 'kmeans_model_2_clusters.pkl': Trained K-Means model.
- 'cluster_labels_2_clusters.pkl': Dictionary mapping cluster indices to genres.
"""
